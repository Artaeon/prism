"""Memory Persistence — Save/load Gunter's complete state.
...
# Default save path
DEFAULT_SAVE_PATH = "gunter_memory.pkl"
...
class MemoryPersistence:
    """Save and load Gunter's complete state.
    ...
    Example:
        ...
        >>> persistence.save(gunter, "my_memory.pkl")
        >>> persistence.load(gunter, "my_memory.pkl")
    """
    
    def save(self, gunter: Gunter, path: str | None = None) -> str:
        """Save Gunter's state to disk.
        
        Args:
            gunter: The Gunter instance to save
            path: File path (default: gunter_memory.pkl)
        ...
        state = {
            ...
            "config": {
                "dimension": gunter.config.dimension,
            },
            
            # Lexicon: only save learned/adjusted vectors
            "lexicon_vectors": _numpy_to_list(dict(gunter.lexicon._vectors)),
            
            # Memory episodes
            "episodes": self._serialize_episodes(gunter.memory),
            "semantic_memory": _numpy_to_list(gunter.memory._semantic),
            "next_id": gunter.memory._next_id,
            
            # User profile
            "user_profile": self._serialize_user_profile(gunter.user_profile),
            
            # Conversation context (last session)
            "conversation": self._serialize_conversation(gunter.context),
        }
    ...
    def load(self, gunter: Gunter, path: str | None = None) -> bool:
        """Load Gunter's state from disk.
        
        Args:
            gunter: The Gunter instance to restore into
            path: File path (default: gunter_memory.pkl)
        ...
        for word, vec_list in saved_vectors.items():
            gunter.lexicon._vectors[word] = np.array(vec_list, dtype=np.float64)
        
        # Restore memory episodes
        self._deserialize_episodes(gunter.memory, state)
        
        # Restore user profile
        self._deserialize_user_profile(gunter.user_profile, state.get("user_profile", {}))
        
        # Restore conversation (optional, may want fresh context)
        conv_data = state.get("conversation", {})
        if conv_data.get("topic"):
            gunter.context.current_topic = conv_data["topic"]
    ...
    def _deserialize_user_profile(self, profile, data: dict) -> None:
        """Restore user profile."""
        from gunter.memory.user_profile import Preference, UserFact
