"""Conversational REPL for Gunter VSA.
...
from gunter.core.lexicon import Lexicon, create_default_lexicon
from gunter.memory import VectorMemory
from gunter.memory.user_profile import UserProfile
from gunter.memory.conversation_context import ConversationContext
from gunter.memory.persistence import MemoryPersistence
from gunter.parsing import SemanticParser
from gunter.parsing.semantic_roles import SemanticRoleParser
from gunter.parsing.user_statement_parser import UserStatementParser
from gunter.reasoning import AnalogyReasoner
from gunter.reasoning.composition import Composer
from gunter.reasoning.transitive import TransitiveReasoner
from gunter.reasoning.confidence import ConfidenceScorer, SourceType
from gunter.reasoning.contradiction import ContradictionDetector
from gunter.reasoning.temporal import TemporalReasoner
from gunter.reasoning.advanced_queries import AdvancedQueryHandler
from gunter.generation import ResponseGenerator
from gunter.generation.explainer import Explainer
from gunter.memory.optimizer import MemoryOptimizer
from gunter.learning.feedback import FeedbackLearner
from gunter.training.trainer import GunterTrainer
from gunter.agents.wikipedia_agent import WikipediaAgent
from gunter.agents.orchestrator import SwarmOrchestrator
from gunter.agents.blackboard import QueryType
from gunter.agents.semantic_router import SemanticRouter, RouteType
from gunter.agents.semantic_weaver import SemanticWeaver
from gunter.agents.quality_gate import ResponseQualityGate
from gunter.reasoning.pattern_library import PatternLibrary
from gunter.reasoning.vsa_reasoner import VSAReasoner
from gunter.reasoning.answer_generator import AnswerGenerator
...
class Gunter:
    """Conversational Gunter VSA assistant."""
    
    def __init__(self, config: VSAConfig | None = None) -> None:
        """Initialize Gunter with all subsystems."""
        ...
        # Training
        self._trainer: GunterTrainer | None = None
    ...
    def process_input(self, text: str) -> str:
        ...
        if typo_suggestion:
            self.context.add_utterance(typo_suggestion, "gunter")
        ...
        if user_result:
            if user_result.handled:
                ...
                self.context.add_utterance(response, "gunter")
                return response
        ...
        if feedback_result:
            self.context.add_utterance(feedback_result, "gunter")
            return feedback_result
        
        # Try advanced queries (list all/count/compare)
        advanced_result = self.advanced.try_handle(text)
        if advanced_result:
            self.context.add_utterance(advanced_result, "gunter")
        ...
        if routed:
            self.context.add_utterance(routed, "gunter")
        ...
        if not self.quality_gate.is_acceptable(response):
            ...
            if swarm_response:
                self.context.add_utterance(swarm_response, "gunter")
    ...
    def _handle_meta_chat(self, text: str) -> str:
        """Handle meta/identity questions about Gunter itself."""
        ...
        if 'name' in t or 'who are you' in t or 'what are you' in t:
            return (
                "I'm Gunter 🧠 — a local AI that learns and reasons using "
        ...
        return (
            "I'm Gunter — a local-first AI assistant. "
    ...
    def _handle_meta(self, text: str) -> str | None:
        ...
        if t in ('and who are you', 'and you'):
            return "I'm Gunter! Ask me anything or type 'help' for commands."
    ...
    def _edit_distance(s1: str, s2: str) -> int:
        ...
        if len(s1) < len(s2):
            return Gunter._edit_distance(s2, s1)
    ...
    def _get_trainer(self) -> GunterTrainer:
        """Lazy-init the trainer."""
        if self._trainer is None:
            self._trainer = GunterTrainer(self)
...
CONVERSATION:
  Just type naturally! Gunter understands:
...
def run_demo(gunter: Gunter) -> None:
    ...
    for description, input_text in demo_inputs:
        ...
        response = gunter.process_input(input_text)
        print(f"Gunter: {response}\n")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Gunter - Conversational VSA Assistant")
    parser.add_argument(
        '--load-knowledge',
        type=str,
        default=None,
    ...
    print("\n🧠 Initializing Gunter (Conversational Mode)...")
    ...
    if args.load_knowledge:
        try:
            ...
            # Transfer loaded data to Gunter's memory
            gunter.memory._semantic = loaded_memory._semantic
            gunter.memory._episodes = loaded_memory._episodes
            gunter.memory._next_id = loaded_memory._next_id
            
            # Restore lexicon vectors
            for word, vec in loaded_memory.lexicon._vectors.items():
                gunter.lexicon._vectors[word] = vec
            
            elapsed = _time.time() - t0
            stats = gunter.memory.get_statistics()
    ...
    persistence = gunter.persistence
    if persistence.load(gunter):
        print(f"   ✓ Loaded previous session ({len(gunter.memory)} facts)")
        if gunter.user_profile.user_name:
            print(f"   ✓ Welcome back, {gunter.user_profile.user_name}!")
    
    print(f"   Lexicon: {len(gunter.lexicon)} words")
    print(f"   Embeddings: {'✓ active' if gunter.lexicon.has_embeddings() else '✗ random'}")
    ...
    # Auto-save on exit
    def _auto_save():
        if len(gunter.memory) > 0 or gunter.user_profile.is_known():
            try:
                persistence.save(gunter)
                print("\n💾 Memory auto-saved.")
            except Exception:
                pass
    ...
    print()
    print(gunter.response_gen.greeting())
    ...
    while True:
        ...
        if cmd in ("quit", "exit", "bye", "goodbye"):
            print(gunter.response_gen.farewell())
            break
        elif cmd == "help":
            print_help()
        elif cmd == "demo":
            ...
            response = gunter.process_input(user_input)
            print(f"Gunter: {response}\n")
