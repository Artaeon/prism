"""PRISM Trainer — Full training pipeline with progress tracking.
...
from prism.training.preprocessor import DataPreprocessor
from prism.training.fact_extractor import FactExtractor, ExtractedFact
from prism.training.validator import FactValidator, ValidationResult
...
class PRISMTrainer:
    ...
    def __init__(self, prism: Any) -> None:
        """Initialize trainer with a PRISM instance."""
        self.prism = prism
        self.preprocessor = DataPreprocessor()
        self.extractor = FactExtractor(nlp=getattr(prism, '_nlp', None))
        self.validator = FactValidator(prism)
    ...
    def train_from_url(
        ...
        try:
            response = requests.get(url, timeout=15, headers={
                'User-Agent': 'PRISM/1.0 (training bot)'
            })
    ...
    def _extract_validate_store(
        ...
        for fact in iterator:
            try:
                self.prism.memory.store(fact.subject, fact.relation, fact.object)
                stored += 1
                
                # Auto-save periodically
                if stored % self.AUTO_SAVE_INTERVAL == 0:
                    try:
                        self.prism.save_memory()
                    except Exception:
                        pass
