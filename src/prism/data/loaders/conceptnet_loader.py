...
facts above a confidence threshold, and maps relations to PRISM format.
...
# Map ConceptNet relations → PRISM relations
RELATION_MAP = {
    '/r/IsA': 'IS-A',
    '/r/HasA': 'HAS',
    '/r/PartOf': 'PART-OF',
...
class ConceptNetLoader:
    ...
    filtering for English-only facts, and maps to PRISM relation types.
    ...
    def _parse(
        ...
        with gzip.open(self._assertions_path, 'rt', encoding='utf-8', errors='replace') as f:
            for line_num, line in enumerate(f):
                ...
                prism_rel = RELATION_MAP.get(relation)
                if not prism_rel:
                ...
                fact_key = (subj_clean, prism_rel, obj_clean)
                if fact_key in seen_facts:
                    skip_reasons['duplicate'] += 1
                    continue
                seen_facts.add(fact_key)

                yield (subj_clean, prism_rel, obj_clean, weight)
                count += 1
